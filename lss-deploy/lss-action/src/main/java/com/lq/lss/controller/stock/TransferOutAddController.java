 ...
 */
@Controller
@RequestMapping(value = "/user/stock/transferOutAdd.htm")

public class TransferOutAddController extends ShiroBaseController<CStockTransfer, String, StockTransferService> {
    ...
    @RequiresPermissions(PermResourceConst.CENTER_TRANS_OUT_ADD)
    @Override
    public ModelAndView save(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            // 调拨流水主表
        	System.out.println("toMchcode"+toMchId);
            MchBaseinfo mchinfoFrom = mchBaseinfoService.queryMchBaseinfoById(toMchId);

            System.out.println("mchinfoFrom.getMchcode()"+mchinfoFrom.getMchcode());
            ...
            cStockTransfer.setFromMchcode("100000");
            ...
            for (CStockTransferDetail detail : dataList) {
                ...
                cStockTransferDetail.setName(detail.getName());
                cStockTransferDetail.setUnit(detail.getUnit());
            ...
            //操作日志
            adminAuditLogService.log(sessionUser.getUserId(), TradeType.STOCK_TRANSFER_OUT.getType(),"新增中心内部调出单申请",cStockTransfer,0L);
