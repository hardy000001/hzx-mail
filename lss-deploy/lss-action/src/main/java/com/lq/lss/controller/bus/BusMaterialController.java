...
@Controller
@RequestMapping("/user/bus/busMaterial.htm")
public class BusMaterialController extends EasyUIController<BMaterialType, Integer, MaterialTypeService>{
	 ...
	 * @return
	 */
	@RequestMapping(params = "method=getMaterialType")
	public ModelAndView getMaterialType(HttpServletRequest request,HttpServletResponse response) {

		
		List<BMaterialType> typeList = materialTypeService.loadAll();
		
		return AjaxModelAndViewUtils.writeMsgReturnNull(response,JSONUtil.toJSonString(typeList));
	}
