...
@Controller
@RequestMapping(value = "/user/stock/centertransfer_inAdd.htm")
public class StockCenterTransferInAddController extends EasyUIController<CStockCenterTransfer, String, StockCenterTransferInService> {
	...
	@Value("/stock/stockCentertransfer_inAdd")
	...
	//加载公司
		...
		@RequestMapping(params = "method=saveCStockCenterTransferInAll")
		@RequiresPermissions(PermResourceConst.CENTER_CENTERTRANSFERIN_ADD)
		public ModelAndView saveCStockCenterTransferAll(HttpServletRequest request, HttpServletResponse response,CStockCenterTransferDto stockCenterTransferDto) {
			...
			try {
				 ...
				 if(stockCenterTransferDto!=null){
					 resultDto= stockCenterTransferInService.saveCStockStockCenterTransferInRdTx(stockCenterTransferDto, user);
					 adminAuditLogService.log(user.getUserId(), TradeType.STOCKCENTERTRANSFER_IN.getType(),"新增调入单",stockCenterTransferDto,0L);
				 }
